...
RESPONSES = {"cilinder_opened": "Motor_opened", "cilinder_closed":"Motor_closed"}
    ...
    def open_close(self, request, response):
        try:
            ...
            if self.check_connection():
                ...
                serialResponse = self.ser.read(self.ser.in_waiting or 1).decode()
                while not serialResponse in RESPONSES.values():
                    time.sleep(0.1) #Wait for a proper response
