...
COMMANDS ={"read_position": "PFB\r", "read_velocity":"MVEL\r", "disable_command":"K\r", 
           "software_enable_command":"en\r", "move_servo_absolute_value":"moveabs", 
           "get_stop_signal":"stopped\r", "set_acceleration": "acc", "set_deceleration":"dec",
           "home_command":"homecmd\r", "home_state_signal":"homestate\r"}
...
class ServoMotorNode(Node):
    ...
    def homing_process2(self, request, response):
        home_command = COMMANDS["home_command"]
        self.read_write_feedback = False
        time.sleep(0.1)
        if self.check_connection():
            ...
            stop_home_signal = 0 
            while stop_home_signal != 24 or stop_home_signal !=26:
                stop_home_signal = self.detect_stop_home_signal()
                self.get_logger().info("Homing still in process.....")

            if stop_home_signal == 26:
                response.success = False
                response.message = f"The homing process for servo motor {self.name} failed!!!"
                self.read_write_feedback = True
                time.sleep(0.1)
                return response
            else:
                response.success = True
                response.message = f"The homing process for servo motor {self.name} was completed!!!"
                self.read_write_feedback = True
                time.sleep(0.1)
                return response    
        else:
    ...
    def read_position(self):
        ...
        if self.read_write_feedback:
            if self.check_connection():
                ...
                self.ser.write(command.encode())
                time.sleep(0.1)
                response = self.ser.read(self.ser.in_waiting or 1).decode().strip().split("\r\n")[1].split(" ")[0]
...
def main():
    ...
    try:
        ...
        servoMotor.get_logger().error("ErrorServoMotorY")
